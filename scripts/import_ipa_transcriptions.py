"""Idempotent IPA-transcription importer for collection_words.

Reads a CSV from content/vocabulary/ipa_transcriptions.csv (or a supplied path)
and patches the `ipa_transcription` column in collection_words.  Values are
stored WITHOUT surrounding slashes — the vocabulary template wraps them at
render time: /{{ word.ipa_transcription }}/.
...
Normalization:
  - Leading/trailing forward-slashes are stripped before storing.
  - Empty values after stripping are rejected.

Idempotency:
  - Words whose ipa_transcription is already set are skipped unless --force.
  - Running twice on the same CSV produces zero changes on the second run.

Flags:
    --dry-run           plan only, no DB writes
    --force             overwrite existing non-null ipa_transcription values
    ...
    FILE                one or more CSV files; defaults to
                        content/vocabulary/ipa_transcriptions.csv

Usage:
    python scripts/import_ipa_transcriptions.py [FILE ...] [options]

Task 37 — see docs/plans/2026-05-11-post-immersion-content-data-plan.md
"""
...
DEFAULT_CSV_PATH = PROJECT_ROOT / "content" / "vocabulary" / "ipa_transcriptions.csv"
...
def normalize_ipa(raw: str) -> tuple:
    """Strip surrounding slashes and whitespace.
    ...
    if not value:
        return None, f"empty IPA value after stripping slashes: {raw!r}"
    return value, None
...
def parse_row(row: dict) -> tuple:
    """Return (IpaEntry, None) or (None, error_str)."""
    word = str(row.get("english_word") or "").strip()
    if not word:
        return None, "empty english_word"

    raw_ipa = row.get("ipa") or row.get("ipa_transcription") or ""
    ipa, err = normalize_ipa(raw_ipa)
    if err:
        return None, err
    ...
    return IpaEntry(english_word=word, ipa=ipa, word_id=word_id, raw_row=dict(row)), None
    ...
    for path in paths:
        try:
            ...
            if reader.fieldnames is None or "english_word" not in reader.fieldnames:
                ...
                continue
            if "ipa" not in reader.fieldnames and "ipa_transcription" not in reader.fieldnames:
                errors.append(f"{path}: missing 'ipa' (or 'ipa_transcription') column")
                continue
...
def _resolve_word(entry: IpaEntry, repo: Repository) -> Optional[object]:
    """Resolve entry to a DB word row, trying word_id first then english_word."""
...
def plan_ipa(
    ...
    *,
    ...
    for entry in entries:
        ...
        old_ipa = getattr(word, "ipa_transcription", None)
        if old_ipa is not None and not force:
            plan.changes.append(
                IpaChange(
                    action="skip_force",
                    english_word=entry.english_word,
        ...
        action = "set" if old_ipa != entry.ipa else "noop"
        plan.changes.append(
            IpaChange(
                action=action,
                english_word=entry.english_word,
...
def format_report(
    ...
    *,
    ...
    lines: list = []
    lines.append("# IPA Transcription Import Report")
    lines.append("")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Import IPA transcriptions into collection_words",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p.add_argument(
        "files",
        nargs="*",
        help="CSV files to import (default: content/vocabulary/ipa_transcriptions.csv)",
    )
    p.add_argument("--dry-run", action="store_true", help="plan only, no DB writes")
    p.add_argument(
        "--force",
        action="store_true",
        help="overwrite existing non-null ipa_transcription values",
    )
    p.add_argument(
        "--report",
        metavar="PATH",
...
def main(argv: Optional[list] = None) -> int:
    ...
    with app.app_context():
        ...
        print(
            f"{prefix}ipa_transcription: {total_set} set, "
            f"{c.get('noop', 0)} noop, "
