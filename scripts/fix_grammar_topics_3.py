...
def emit_sql(all_changes: dict[str, list[dict]], theory: dict[str, tuple[dict, dict]]) -> dict[str, str]:
    ...
    for topic, changes in all_changes.items():
        ...
        for change in changes:
            ...
            check_rows.append(f"SELECT {_s(label + ' old')} AS slot, count(*) AS found FROM grammar_exercises "
                              f"WHERE {_where_exercise(slug, old)}")
            check_rows.append(f"SELECT {_s(label + ' new')} AS slot, count(*) AS found FROM grammar_exercises "
                              f"WHERE {_where_exercise(slug, new_row)}")
            if change['kind'] == 'fix':
                apply.append(
