...
def create_app(config_class=Config):
    ...
    def update_last_active():
        """Update user's last_login once per hour on activity"""
        from flask_login import current_user
        from datetime import datetime, timezone, timedelta

        if current_user.is_authenticated:
            now = datetime.now(timezone.utc)
            # Update if last_login is None or older than 1 hour
            if current_user.last_login is None or \
               (now - current_user.last_login.replace(tzinfo=timezone.utc)) > timedelta(hours=1):
