...
| **Task 9 — отобранные P2/P3** | UI: `UI-008` · `UI-010` · `UI-011` · `UI-012` · `UI-013` · `UI-015` · `UI-016` · `UI-017` · `UI-009` (scoped) · `UI-022` · `UI-024` · `UI-029` · `UI-031` (dead-code) · `UI-035` · `UI-036` · `UI-037` — Контент: `CNT-006` · `CNT-007` · `CNT-010` · `CNT-011` · `CNT-012` · `CNT-013` · `CNT-014` · `CNT-019` · `CNT-026` — Админка: `ADM-002` · `ADM-007` · `ADM-008` | 28 | ✅ 26 закрыто · 🟡 `UI-029` · ⬜ `CNT-006` → отложена |
| **Отложено** | 41 находка по таблице ниже + `CNT-006` (перенесена в Task 9) | 42 | ⬜ открыто |
...
| `CNT-003` | 🟡 | Смягчение, как и планировалось: `audioPlayer.play()` получил `.catch()` → `markAudioUnavailable()` гасит кнопку (`.audio-btn-card--unavailable`); `scripts/clear_missing_word_audio.py` обнуляет `listening`/`get_download` там, где файла нет (внешние URL — fail open). **Сами 1570 клипов требуют внешнего TTS — Post-Completion** | `463602bb` |
...
| `CNT-006` | ⬜ | **Не закрыт — см. отклонение №1.** Снятие филлера уводит все 196 примеров ниже планки «контекстного примера» для B1+ | — |
...
1. **`CNT-006` остался открытым, хотя стоял в составе задачи.** Правка была применена, дала
   ...
   **Что это значит для реестра:** планка «контекстных примеров» на B1+ сегодня выполняется
   генераторной заглушкой, а не содержанием. Это находка крупнее `CNT-006` и она записана здесь,
   а не растворена в статусе «частично». Правило скрипта сделано уровне-зависимым, так что в
   свежем checkout'е филлер снимается только там, где пример и без него полноценен.
...
B1+ ниже планки контекстного примера; разбор — отклонение №1 в «Task 9»). Итог: **42 отложенных**,
`17 + 27 + 42 = 86` — сходимость сохранена.
