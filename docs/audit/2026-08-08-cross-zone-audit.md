...
| SEC-001 | ✅ | ✅ | ⚠️ | **P1** | Кандидат на P0 рассмотрен и отклонён по букве критерия (утекает лицензионный материал, не данные пользователя) — см. врезку в «Сводке severity». В Task 8 идёт первой **Поправка после ревью Task 10:** «задача без блока → 404 (fail closed)» оказалось неверным для daily-lesson задач — `Task.block_id` nullable by design, и они доходят до книги через `task.daily_lesson.chapter.book`. Введён `_task_gate`; `_block_gate` остаётся для роутов по `block_id`. Тем же проходом загейчен `/api/book/<id>/content` — последний роут файла, резолвивший `Book` мимо проверки |
...
| **Task 9 — отобранные P2/P3** | UI: `UI-008` · `UI-010` · `UI-011` · `UI-012` · `UI-013` · `UI-015` · `UI-016` · `UI-017` · `UI-009` (scoped) · `UI-022` · `UI-024` · `UI-029` · `UI-031` (dead-code) · `UI-035` · `UI-036` · `UI-037` — Контент: `CNT-006` · `CNT-007` · `CNT-010` · `CNT-011` · `CNT-012` · `CNT-013` · `CNT-014` · `CNT-019` · `CNT-026` — Админка: `ADM-002` · `ADM-007` · `ADM-008` | 28 | ✅ 27 закрыто (`CNT-006` — 2026-08-24) · 🟡 `UI-029` |
| **Отложено** | 41 находка по таблице ниже (`CNT-006` вернулась в Task 9 и закрыта 2026-08-24) | 41 | ⬜ открыто |
...
- `CNT-003` — планировалось как частичное закрытие «1570 клипов требуют внешнего TTS». **Хвост
  закрыт 2026-08-23 перепроверкой на проде: находка меряла диск dev-чекаута, в проде клипы есть у
  всех слов с поднятым флагом** (300/300). Скриптовое смягчение осталось в силе; TTS не нужен —
  озвучка пополняется вручную живыми дикторами. Детали — во врезке в «Статусе ремедиации».
...
### Task 8 — P1: закрыта (2026-08-08)

Состав по «Плану ремедиации»: **16 P1 + `CNT-004`** как предусловие приёмки. Коммитов девять,
по группам плана. Каждая правка несёт тесты (**86 новых тестов в 12 файлах**).

| ID | Статус | Что сделано | Коммит |
|---|---|---|---|
| `SEC-001` | ✅ | `_block_gate()` в `app/api/books.py` — три роута (`/tasks/<id>`, `/blocks/<id>`, `/blocks/<id>/tasks`) зовут `_draft_hidden` + `can_user_access_book`, как их 5 соседей. Задача без блока → 404 (fail closed) | `5f11f4f1` |
| `SEC-002` | ✅ | `_safe_back_url()` / `_same_host_path()` в `app/books/routes.py` — `?from=` и `Referer` обеих веток ридера идут через `get_safe_redirect_url` | `5f11f4f1` |
| `CNT-001` (+ поглощённая `SEC-005`) | ✅ | Два обработчика финального теста сведены в один: `final_test_lesson` — тонкая обёртка над `render_final_test_lesson`; общий сборщик формы `_collect_final_test_form()` читает `pairs_<i>`; редирект «попытки исчерпаны» → существующий `curriculum_lessons.final_test_lesson` | `d83929dd` |
| `SEC-003` | ✅ | `study.deck_detail` (не существует) → `study.edit_deck` в `components/_flashcard_session.html` | `1613d2d4` |
| `UI-001` | ✅ | Создан `auth/change_password.html` на `base.html` + таксономии `pf-*`; POST на успехе делает redirect на профиль; пароль проверяется общим `validate_password_strength` — как в register и reset | `9e651a0a` |
| `UI-002` | ✅ | `admin/base.html` объявляет `{% block extra_js %}` (имя как в `base.html` / `public_base.html`). Побочно починены два шаблона, которые вообще не парсились: `admin/curriculum/content/card_view.html` (обрезанная голова, 500 на просмотре card-урока) и `admin/curriculum/srs_settings.html` (нет `{% block content %}`) | `9e651a0a` |
| `UI-004` | ✅ | `/words`: выбор считается по **уникальным id** (`getSelectedWordIdsUnique`), близнецы синхронизируются (`syncTwinCheckbox`), у мобильного списка появился свой `#selectAllMobile` | `7e983708` |
| `UI-005` | ✅ | `enhancedBulkActionsSetup` в `main.js` больше не подменяет узлы `cloneNode`-копиями — `bindOnce()` по `dataset.bulkBound`; шаблон перешёл на делегированный `change` | `7e983708` |
| `UI-003` | ✅ | 5 book-course шаблонов: `.catch` зовёт новый `showSaveError(retry)` вместо `showCompletionUI()` — сообщение «Урок не засчитан» + кнопка «Повторить», собранные через DOM API. Стиль `.lesson-save-error` в `design-system.css` | `3ca9207a` |
| `UI-007` | ✅ | `comprehension_check` / `match_headings` / `context_review`: `role="button"` + `tabindex` + Enter/Space на всех кликабельных `div`; `selectOption` не даёт переответить (паритет с `pointer-events:none`) | `3ca9207a` |
| `ADM-001` | ✅ | Книжный поиск переехал на `/admin/api/book-courses/words/search`; `/admin/api/words/search` резолвится в `admin.api_words_search` (проверено живым `url_map.match`) — автокомплит колод снова видит все 25 089 слов, а не 12 767 | `ab5c8320` |
| `CNT-004` | ✅ | `_iter_audio_refs` полностью рекурсивен + понимает `audio: [...]`; `url` остался только верхнеуровневым, чтобы вложенные картинки не считались аудио. **Проверено на корпусе: 1397 → 8274 ссылки, `missing=0` → `missing=1570`** — ровно число `CNT-003` | `463602bb` |
| `CNT-003` | ✅ | Смягчение, как и планировалось: `audioPlayer.play()` получил `.catch()` → `markAudioUnavailable()` гасит кнопку (`.audio-btn-card--unavailable`); `scripts/clear_missing_word_audio.py` обнуляет `listening`/`get_download` там, где файла нет (внешние URL — fail open). **Хвост «1570 клипов» снят 2026-08-23 как артефакт окружения — см. врезку ниже** | `463602bb` |

**Перепроверка `CNT-003` на проде (2026-08-23) — находка не воспроизводится.**
Цифра «1570 отсутствующих» получена по диску dev-чекаута, где лежит сэмпл из **33** файлов
`pronunciation_*.mp3`; медиатека живёт на проде. Замер против `https://llt-english.com/static/audio/`:

| Проверка | Результат |
|----------|-----------|
| Случайные 300 слов с `get_download=1` (кнопка рисуется) | **300/300** отдают `audio/mpeg` |
| Случайные 300 ссылок `listening` без учёта флага | 239 есть · 61 нет (20.3%) |
| Флаг у тех самых 61 отсутствующих | `get_download=0` (59) · `NULL` (2) · **`=1` — ни одного** |
| Контроль на ложный 200 | несуществующее имя → настоящий `404 text/html` |

Флаг и файлы согласованы в обе стороны: нет файла → нет кнопки. Мёртвых кнопок на проде нет.
Админская сводка (19 890 с аудио / 5 199 без, 20.7%) считается по `get_download`, а не по наличию
файла, и совпала с выборкой. **Оставшиеся 5 199 слов без озвучки — не баг:** аудио пополняется
вручную через сервис с живыми дикторами, TTS-генерация для слов не применяется.

> ⚠️ **Побочный вывод — `scripts/clear_missing_word_audio.py` был миной.** Он сравнивает БД с
> локальной папкой и при `--apply` пишет в ту БД, на которую смотрит конфиг, а локальная — копия
> прода. Прогон из чекаута обнулил бы `listening` у ~1 500 слов, чьи клипы в проде есть и куплены
> вручную. Добавлен гейт покрытия (`COVERAGE_FLOOR = 0.9`): `--apply` отказывается работать, если в
> папке меньше 90% файлов, на которые ссылается БД, и объясняет, что это не «нет аудио», а «не та
> машина; обход — явный `--force`. Проверено сквозным прогоном на одноразовой SQLite-базе: отказ при
> 0% и 67% не пишет ничего, `--force` чистит ровно одну реально отсутствующую строку.
| `CNT-002` | ✅ | `scripts/fix_reading_matching_exercises.py` разворачивает `matching` в по одному `multiple_choice` на пару. **14/14 упражнений сконвертированы**, правильный ответ распределён по слотам (0:29 / 1:15 / 2:14 / 3:13), литерал `False` в `data-correct` исчез | `9772e349` |
| `CNT-005` | ✅ | Миграция `20260808_level_entry_prerequisites` проставляет входным модулям уровней prereq на последний модуль предыдущего. Чтобы планка совпала с внутриуровневой, `check_prerequisites` понимает `min_progress` (default 100 — авторские prereq'ы не ослаблены). Placement не задет: `min_level_order` по-прежнему пропускает нижние уровни | `9772e349` |
| `CNT-008` | ✅ | `scripts/fix_borrowed_lesson_audio.py` снял **5/5** заимствованных ссылок (`A2M22L6_ex1..4` из `A2_7 L8`, `A2M19L12_listen_1` из `A2_4 L18`). Владелец определяется только среди модулей, реально делящих клип; клипы `pronunciation_*` и легаси-нумерация не трогаются | `9772e349` |
| `CNT-009` | 🟡 | `scripts/fix_mail_merge_dialogues.py` удалил **3 доказуемо сломанных** упражнения. Детектор намеренно узкий — срабатывает только на эхо слота из вопроса в ответ (`verb_pair_echoed`, `adjective_echoed_without_head_noun`). **Остальные ~107 кандидатов исходной регулярки — семантика, требует автора; не удалены** | `9772e349` |

**Отклонения от плана — зафиксированы, а не замолчаны:**

1. **`UI-004` починен дедупликацией, а не «одним `.word-checkbox` на слово».** План предлагал тест
   «`.word-checkbox` ровно по одному на слово»; это потребовало бы слить таблицу и мобильные карточки
   в один рендер — правка на порядок крупнее и с риском для всей вёрстки `/words`. Оба узла оставлены,
   считаются и отправляются уникальные id, состояние близнецов синхронизировано. Симптомы находки
   («40 слов выбрано» при 20 строках, дубли id в запросе, отсутствие «выбрать все» на мобиле) закрыты
   полностью; двойной вес DOM остаётся как долг.
2. **`CNT-009` закрыт на 3 из ~110.** Воспроизвести исходную регулярку означало бы удалять хорошее:
   на дословных проверках она сносит «a piece of cake — easier than I thought», «once in a blue moon»
   и «found a wallet on the street — what would you do?». Детектор сужен до механической подписи
   mail-merge; всё остальное оставлено человеку и **не** записано как закрытое.
3. **`CNT-005` потребовал правки кода, а не только миграции.** Формат prereq'ов умеет лишь
   «100% пройдено»; поставить входным модулям такой гейт означало бы планку строже, чем где-либо
   в продукте (внутри уровня — 80%). Добавлен `min_progress` с default 100.
4. **Починены два дефекта вне состава Task 8** — `card_view.html` и `srs_settings.html` не
   парсились Jinja вовсе. Их нашёл тест, написанный для `UI-002`; оставить их означало бы либо
   красный тест, либо молча пропускающий (в реестре таких находок нет — это тот самый
   несостоявшийся хенд-офф Task 2 ↔ Task 5, который критик-агент назвал в Task 6).
5. **Правлен один существующий тест** — `test_db_error_during_password_change_logs_exception`
   слал пароль `NewPass123!@#`, который общий `validate_password_strength` отклоняет за
   последовательность «123», поэтому до проверяемого `commit` управление больше не доходило.
   Заменён литерал пароля; предмет теста (логирование исключения) не тронут.

**Гейт задачи:** полный `pytest` — **26 падений против 27 в baseline, новых падений 0**
(`comm -13` пусто). Позеленел `tests/srs/test_counting.py::TestCountRestingWords::test_ignores_short_session_bury`
— это известная зависимость от часа старта (разбор в Task 7), не эффект правок.

⚠️ **Первый прогон дал одну регрессию, и она была настоящей.**
`tests/test_auth_routes.py::TestAuthExceptionHandling::test_db_error_during_password_change_logs_exception`
упал из-за правки `UI-001` (пароль теста не проходил новую проверку стойкости и до `commit` не
доходил). Починено, прогон повторён — отклонение №5 выше. Записано здесь, а не убрано, потому что
гейт «дифф против baseline» сработал ровно так, как задуман.

### Task 9 — отобранные P2/P3: закрыта (2026-08-08)

Состав по «Плану ремедиации»: **28 ID**. Коммитов четыре, по группам плана; каждая правка несёт
тесты (**168 новых тестов в 6 файлах**).

| ID | Статус | Что сделано | Коммит |
|---|---|---|---|
| `CNT-011` | ✅ | `validate_corpus.py` кладёт корень проекта в `sys.path` **до** импорта `LessonContentValidator`, а недоступность схемы стала блокирующей ошибкой вместо примечания. Штатный `python scripts/validate_corpus.py` теперь реально прогоняет рантайм-схему: 0 errors | `0006d049` |
| `CNT-012` | ✅ | Пустой ведущий `prompt` у `sentence_completion` легален, когда текст несут `context`/`prompt_after` (так и рендерит шаблон). Флагом остаётся только строка без единого текста. **29 ложных ошибок → 0** | `0006d049` |
| `CNT-013` | ✅ | `create_app()` вместо `create_app("development")` и `app.utils.db.db` вместо корневого `extensions.db` — в обоих аудиторах. Прогнан против `learn_db_prod`: **86/86 модулей, 1548/1548 уроков по `external_key`, 0 json-only / 0 db-only / 0 коллизий** — то, что Task 3 добывал обходом, теперь даёт сам скрипт | `0006d049` |
| `CNT-026` | ✅ | 148 заглушек «Перевод фразы из словаря модуля.» удалены ключом целиком (оба рендерера гейтят на `if (question.explanation)`), в JSON и в БД | `73c330be` |
| `CNT-007` | ✅ | 174 строки чтения очищены от «This reading version adds context one.» — **и вместе с ними сняты 174 обнажившихся дубля** (см. отклонение №2) | `73c330be` `f8800246` |
| `CNT-006` | ✅ | **Закрыт 2026-08-24 авторской правкой.** Скриптом он не закрывался в принципе: измерено на корпусе — все 196 из 196 после снятия филлера падают ниже своего порога (B1 6 слов, B2/C1 7). `scripts/rewrite_short_vocab_examples.py` несёт 196 написанных примеров с русскими переводами: исходная фраза сохранена и достроена, слово карточки на месте. Планку теперь держит содержание, а не заглушка | — |
| `CNT-010` | ✅ | `scripts/fix_writing_prompt_scaffolding.py` снял стартовый A1-каркас с **32/32** `writing_prompt` (13 A1 + 19 A2): `target_phrases` (гейт завершения) убран везде, генераторный `template` — на 30 (два авторских оставлены), из `hint_words` вырезан только общий префикс. Применено к JSON и БД | `0db69753` |
| `CNT-014` | ✅ | Миграция `20260808_normalize_module_prerequisites` резолвит слаги `module_<n>` в `modules.id` **внутри того же уровня** и переписывает их в форму, которую читает `check_prerequisites`. Нераспознанный слаг сохраняется как есть (авторский замысел не теряется), downgrade восстанавливает исходные строки | `0db69753` |
| `CNT-019` | ✅ | Миграция `20260808_clear_null_literal_word_lists` — 1728 `antonyms` + 180 `synonyms` с литералом `["null"]` → `NULL` (established «нет данных»: так уже у 20 146 строк). Перед правкой проверено: других мусорных форм в таблице нет | `0db69753` |
| `ADM-002` | ✅ | Гейт покрытия аудит-логом выводится из `url_map` (всё под `/admin`), а не из литерального списка префиксов: **15 префиксов → 16 неймспейсов, 115 мутирующих правил**. Все 7 невидимых роутов получили аудит: 6 telegram-канальных (`channel_post.*`) и массовая рассылка (`reminder.send_campaign`). Три необратимых пишут запись **до** отправки | `96a2bf6e` |
| `ADM-007` | ✅ | `log_admin_action` на 5 экспортах, включая экспорт самого аудит-лога. **Плюс дефект уровнем ниже** (отклонение №3): два «уже покрытых» экспорта стейджили строку и не коммитили — на GET коммитить больше нечего, и запись не переживала teardown | `96a2bf6e` |
| `ADM-008` | ✅ | `escape_like` + `escape='\\'` на всех 4 живых `ilike` (`main_routes`, `curriculum_routes`, `quiz_decks`, `book_courses`). Тест поведенческий: `Present_Simple` больше не матчит `Present Simple`, а сырая форма — матчит | `96a2bf6e` |
| `UI-008` | ✅ | `_('…')` уехал в браузер дословно → `ReferenceError` убивал `catch`. Строка переведена на серверный `{{ … \| tojson }}` в обоих шаблонах; плюс `catch` возвращает скрытый setup-UI, иначе экран оставался пустым | `f8800246` |
| `UI-011` | ✅ | Заметка к слову и добавление в список: `resp.ok`, разбор ошибочного JSON, видимое сообщение (`.word-annotation__error` с `role="alert"`) вместо пустого `catch` | `f8800246` |
| `UI-012` | ✅ | «Завершить теорию»: `response.ok`, дизейбл на время запроса, видимая ошибка. Плюс кнопка теперь оседает и при повторном клике (`xp_earned = 0` — это успех, а не молчание) | `f8800246` |
| `UI-010` | ✅ | Оценка и балл пишутся **после** раскрытия контейнера (`requestAnimationFrame`), фокус переносится в панель (`tabindex="-1"`) — до этого мутация шла в `display:none`, и live-region молчал | `f8800246` |
| `UI-013` | ✅ | Снят `body.style.paddingTop = '48px'` — сдвиг даёт CSS-правило на navbar. Было 96px и полоса фона в 48px | `f8800246` |
| `UI-015` | ✅ | Меню «Ещё» в bottom-nav: `role="button"`, `tabindex`, `aria-expanded`/`aria-haspopup`, Enter/Space, Escape, фокус на первый пункт | `f8800246` |
| `UI-016` | ✅ | Модалка level-up: `role="dialog"`, `aria-modal`, `aria-labelledby`, фокус на кнопку при открытии, Escape, ловушка Tab, возврат фокуса на триггер. Оба места показа переведены на `openLevelupModal()` | `f8800246` |
| `UI-017` | ✅ | Один шим в `base.html` снимает `behavior:'smooth'` при `prefers-reduced-motion` — закрывает все 31 сайт и любой будущий, вместо правки 31 места (см. отклонение №4) | `f8800246` |
| `UI-022` | ✅ | `sentence_completion` обновляет `aria-valuenow` (было навсегда `0`); в `books/details.html` бар читал ключ `queued`, которого в `word_stats` нет, — теперь `learning`, то самое число, которое он рисует | `f8800246` |
| `UI-024` | ✅ | Single-select переведён с `aria-pressed` на `radiogroup`/`aria-checked` (обе ветки `sentence_correction`); `listening_immersion` получил `role="radio"` детей к своей осиротевшей группе; matching-кнопки финального теста наконец сообщают выбор | `f8800246` |
| `UI-029` | 🟡 | 13 литералов файлов плана дня (`linear-plan-context.js`, `linear-daily-plan.js`) routed через `window.I18N` с русским фолбэком — рендер сегодня байт-в-байт прежний. **`flashcard-session.js` (67 литералов) не брался** — см. отклонение №5 | `f8800246` |
| `UI-009` | ✅ | Scoped, как и планировалось: обзёрвер на месте (он несущий для `/study/cards`), но `flashcard-session.js` помечает контейнер `data-plan-cta-source="inline"` **до** раскрытия экрана, и обзёрвер отходит в сторону. Гонка и hard-redirect поверх отрисованных CTA сняты | `f8800246` |
| `UI-031` (+ `UI-034`) | ✅ | `daily-plan-next.js` удалён целиком вместе с 3 подключениями, 4 контейнерами, 6 осиротевшими диспатчерами `dailyPlanStepComplete`, CSS-правилом и Jinja-флагом `daily_plan_next_enabled`. Полярность перепроверена самостоятельно перед удалением (см. отклонение №6) | `f8800246` |
| `UI-035` | ✅ | 5 JS-файлов удалены (0 ссылок). Побочно снят вечно пустой `#processing-status` в `books/details.html` — его наполнял только удалённый `book_processing.js` | `f8800246` |
| `UI-036` | ✅ | 9 CSS-файлов удалены (~200 KB). Ноль ссылок подтверждён и по `filename='css/…'`, и по `@import` внутри самих CSS | `f8800246` |
| `UI-037` | ✅ | 5 шаблонов-сирот удалены. Динамический `render_template(f'…/{template_name}.html')` в `book_courses.py` разобран отдельно — его имена берутся из закрытого списка и до удалённых не дотягиваются | `f8800246` |

**Отклонения от плана — зафиксированы, а не замолчаны:**

1. **`CNT-006` остался открытым, хотя стоял в составе задачи.** Правка была применена, дала
   регрессию в `test_advanced_levels_have_contextual_examples` и была отозвана. Причина не
   техническая: `tests/test_module_content_quality.py` требует, чтобы примеры лексики B1+ были
   развёрнутыми (≥6 слов на B1, ≥7 на B2/C1), и **у всех 196 филлер — единственное, что эту планку
   держит**; реальный пример под ним — 3–6 слов («Urban life is very busy.», «I work downtown.»).
   Соответствие измерено: 196 примеров в 33 модулях по обе стороны. Выбор был между
   «оставить недопереведённую карточку» и «сломать инвариант качества контента»; третий путь —
   написать 196 полноценных примеров — авторская работа, прямо вынесенная планом за скобки.
   **Что это значило для реестра:** планка «контекстных примеров» на B1+ выполнялась генераторной
   заглушкой, а не содержанием. Это находка крупнее `CNT-006`, и она была записана здесь, а не
   растворена в статусе «частично». Правило скрипта сделано уровне-зависимым, так что в свежем
   checkout'е филлер снимался только там, где пример и без него полноценен.
   **Закрыто 2026-08-24** тем самым третьим путём: 196 примеров написаны (`scripts/
   rewrite_short_vocab_examples.py`), каждый сохраняет исходную фразу, достраивает её до порога
   уровня и несёт синхронно расширенный русский перевод. Проверки: `test_module_content_quality.py`
   — 9 passed, `validate_corpus.py` — 0 errors / 0 warnings, повторный прогон скрипта — 196
   «already applied». Филлера в корпусе не осталось, планку держит содержание.
2. **`CNT-007` потребовал удалять строки, а не только подстроку.** Филлер оказался единственным
   различием между целыми повторяющимися блоками: в `module_B2_10_crime_and_law` строки 23–34 —
   это строки 3–14 с приклеенным «…adds context one.». Снять только хвост означало показать
   ученику одни и те же 12 предложений дважды подряд. Правило дополнено удалением строки, ставшей
   точным дублем более раннего. Проверено **до** правки, что это не заденет чужие данные: дубли
   строк чтения есть ровно в тех же 18 модулях и в том же числе (174), что и филлер.
3. **`ADM-007` вскрыл дефект уровнем ниже заявленного.** Реестр писал, что 2 экспорта из 7 аудит
   пишут, а 5 — нет. На деле не писал **ни один**: `log_admin_action` только стейджит запись в
   savepoint и требует коммита от вызывающего, а это GET-хендлеры, которым коммитить нечего, —
   строка не переживала teardown сессии. Явный `db.session.commit()` добавлен во все семь.
4. **`UI-017` закрыт одним шимом, а не правкой 31 места.** Правка каждого вызова оставила бы
   дефект открытым для следующего добавленного `scrollIntoView`; шим в `base.html` снимает
   `smooth` на уровне `Element.prototype` ровно тогда, когда пользователь просит меньше движения.
   Цена честная: это патч прототипа DOM, и он назван таковым в комментарии.
5. **`UI-029` закрыт частично, по границе, которую план и задал.** Взяты файлы плана дня
   (13 литералов); `flashcard-session.js` с 67 литералами не брался — это отдельный заход,
   несоразмерный P3-долгу при одной поставляемой локали.
6. **Удаление `daily-plan-next.js` шло против двух тестов, которые запрещали удалять.**
   `test_discrete_bugfixes.py` утверждал «файл ЖИВОЙ — НЕ удалять», `test_completion_contract.py` —
   «ридер по-прежнему подключает». Полярность перепроверена самостоятельно, а не по реестру:
   гейт требует `?from=daily_plan`; единственный эмиттер в `app/` — `phrase_review.py:207`
   (`/learn/phrase-review/`); этот роут рендерит `curriculum/phrase_review.html`, который наследует
   `base.html` и скрипт не подключает; все URL плана несут `from=linear_plan`. Оба теста
   переписаны с указанием, что именно в прежней посылке было неверно.
7. **Правлены три существующих теста** — `test_option_btn_classes_present` (ждал `aria-pressed`,
   который `UI-024` заменил на `aria-checked`), плюс два выше. Ни один не ослаблен: в каждом
   ассерт заменён на проверку нового, более строгого контракта.

**Гейт задачи:** полный `pytest` — **26 падений против 27 в baseline, новых падений 0**
(`comm -13` пусто); `pytest -m smoke` — все зелёные, 0 падений; `python -c "import app"` — чист;
`scripts/validate_corpus.py` — 0 errors / 0 warnings.

⚠️ **Первый прогон дал три настоящие регрессии, все три — от контентных правок**
(`test_option_btn_classes_present`, `test_advanced_levels_have_contextual_examples`,
`test_generated_content_is_not_repeated_verbatim`). Две последние оказались не шумом, а сигналом:
филлер маскировал и короткие примеры, и дублированные блоки чтения. Разбор — отклонения №1 и №2.

**Про `ruff`.** На входе в Task 9 (`7019f67f`) тот же бинарь даёт **4594**, а не 4582 из Task 1 —
расхождение возникло до этой задачи. После Task 9 — **4593**, то есть на одно меньше. Новых
нарушений задача не внесла; несоответствие числа из Task 1 записано здесь, а не подогнано.

---

### Task 10 — приёмка (2026-08-08)

Пять критериев приёмки прогнаны на чистом дереве от `2320bc46`. Результаты — в «Verification» ниже;
здесь — то, что прогон вскрыл и потребовало правки.

| Критерий | Результат | Комментарий |
|---|---|---|
| Полный `pytest` vs baseline | ✅ 26 падений, **новых 0** | `diff` против baseline дал только строку `<` |
| `pytest -m smoke` | ✅ 684 теста, 0 падений | exit 0 |
| `ruff check .` | ✅ **4579 ≤ 4582** | было 4593 — см. ниже |
| `python -c "import app"` + `validate_corpus.py` | ✅ импорт чист, 0 errors / 0 warnings | 86 модулей, 1548 уроков |
| Финальный статус у каждой находки | ✅ **86 из 86** | 41 ✅ + 3 🟡 + 42 ⬜; парсер по таблицам |

**Единственная правка задачи — снятие лишних нарушений ruff.** Первый прогон дал **4593 против
базовых 4582**, то есть +11. Локализовано не оценкой, а диффом per-file против рабочего дерева на
`740d80d5` (том самом коммите, на котором снят baseline; контрольный прогон там воспроизвёл 4582
ровно). Новых нарушений было **14**, минус 3 снятых попутно ветвью (`app/auth/routes.py` −2 в Task 8,
удалённый `tests/test_js_daily_plan_error_handling.py` −1):

| Файл | Нарушений | Откуда |
|---|---|---|
| `app/feedback/survey.py` | 8 (UP035, UP007 ×4, UP017 ×2, SIM103) | `5e040ff7` — фича опроса, **не** ремедиация |
| `app/feedback/models.py` | 1 (RUF001, знак `×` в `__repr__`) | `5e040ff7` |
| `tests/test_survey.py` | 1 (UP017) | `5e040ff7` |
| `tests/curriculum/test_final_test_matching_submission.py` | 2 (UP017) | `d83929dd` — Task 8 |
| `tests/study/test_deck_nothing_to_study.py` | 2 (UP017) | `1613d2d4` — Task 8 |

Все 14 сняты. Стилевого расхождения это не создаёт: `datetime.UTC` в проекте **уже** принятая форма
(`app/curriculum/*` — десяток файлов), Python здесь 3.14.5. Предсуществующие 6 UP017 в
`app/feedback/models.py` **не** трогались — baseline прямо требует «не уменьшать специально».
Единственная правка с логикой — SIM103 в `should_show_survey`: `if …: return False / return True`
свёрнут по Де Моргану в `return dismissed_at is None or moment - dismissed_at >= …`. Обе ветви
покрыты прошедшими тестами (`test_first_dismissal_hides_it_for_a_week`,
`test_it_returns_once_after_the_snooze`).

⚠️ **Оговорка о происхождении долга.** 10 из 14 нарушений пришли из коммита `5e040ff7`, к аудиту
отношения не имеющего, — он просто лежит на этой же ветке. Гейт «ноль новых против Task 1» их
всё равно ловит, поэтому они сняты здесь, а не отнесены к чужой работе.
...
- Полный `pytest`, диффом против `docs/audit/2026-08-08-baseline-pytest.txt` — **ноль новых падений**
  ...
  **Финальный прогон Task 10: собрано 10 261, падений 26, новых 0** (`diff` дал только строку `<`).
- ⚠️ **Один nodeid baseline зависит от часов** —
  `tests/srs/test_counting.py::TestCountRestingWords::test_ignores_short_session_bury`
  (порог «resting» = 02:00 завтра по `Europe/Moscow`). **Окно, записанное в Task 7 («падает при
  старте в 19:00–23:00 UTC»), не подтвердилось:** прогон Task 10 стартовал в **20:11 UTC**, то есть
  внутри этого окна, и тест **прошёл**. Зависимость от стенных часов реальна, но конкретная граница
  в Task 7 описана неверно — опираться на неё при разборе регрессий нельзя, сверять нужно поведением,
  а не временем. Из baseline не вычёркнут намеренно (правило baseline-файла: список задним числом
  не переписывается).
- `pytest -m smoke` — **684 теста, 0 падений, exit 0** (Task 10); baseline здесь зелёный, поэтому
  любое падение = регрессия.
- `ruff check .` — **4579 ≤ 4582** (Task 10), тем же бинарём 0.5.6 из кэша pre-commit.
- `python -c "import app"` — импорт чист (Task 10, включая правленые модули `app/feedback/`).
- `scripts/validate_corpus.py` — **0 errors / 0 warnings**, 86 модулей / 1548 уроков / 8269 аудио-ссылок (Task 10).
- Каждая находка реестра имеет финальный статус (✅ / 🟡 / ⬜) — **проверено парсером в Task 10:
  86 из 86, без пропусков и без противоречий**.
...
>
> **Снятие оговорки (Task 9).** Четыре из пяти гейтов починены: `CNT-004` (Task 8), `CNT-011`,
> `CNT-012`, `CNT-013` и `ADM-002` (Task 9). С этого момента «`validate_corpus.py` — 0 errors»
> и зелёный `test_audit_log_coverage.py` снова что-то утверждают: первый реально прогоняет
> рантайм-схему, второй видит 16 неймспейсов и 115 мутирующих правил вместо 15 литеральных
> префиксов. Оговорка остаётся в силе **только** для `ADM-006` — `test_legacy_admin_routes.py`
> по-прежнему ходит по шести URL, которые обслуживает не тот файл, что он проверяет
> (находка отложена, см. «Отложено»).

### Отложенные P2/P3 — подтверждение после Task 9

Список Task 6 («Отложено — 41 находка») в силе целиком: ни одна из этих находок в Task 9 не
трогалась, причины не изменились. **Изменение одно: `CNT-006` переходит из состава Task 9 в
отложенные** — с причиной, которой в Task 6 знать было нельзя (снятие филлера роняет 196 примеров
B1+ ниже планки контекстного примера; разбор — отклонение №1 в «Task 9»). После закрытия
`CNT-006` 2026-08-24 итог: **41 отложенная**, `17 + 28 + 41 = 86` — сходимость сохранена.

Новая причина отказа, которой не было в таблице Task 6:

| Группа | ID | Причина |
|---|---|---|
| Фикс упирается в другой инвариант качества | `CNT-006` | Единственная деталь, держащая планку «контекстных примеров» на B1+, — сама заглушка. Закрытие требует написать 196 примеров (авторская работа), а не удалить строку |

### Второй проход код-ревью — правки поверх Task 8–10 (2026-08-09)

Ревью ветки нашло, что связка `CNT-005` + `CNT-014` + новое различение «пройден / заблокирован»
в `find_next_lesson_state` образует тупик, которого до ветки не было. Правки ниже закрывают его
и три смежных дефекта. Все — поверх уже закрытых находок, новых ID реестру не добавляют.

| Что | Почему это дефект | Правка |
|---|---|---|
| **Заблокированный спайн не давал закрыть день** | `graduated` теперь требует `blocking_module_id is None`, но `required` при блоке всё равно `[]`, а `compute_day_secured_from_activity` для `required=[] and not graduated` возвращал `False` **безусловно**. Учащийся с невыполненным prereq'ом мог заниматься весь день и не закрыть его никогда: streak, ранги, perfect-day замерзали. До ветки такой юзер (ошибочно) считался graduated и день закрывал | `_plan_meta.blocked_module_id` + payload `blocked_module` (id/номер/название/уровень/`reasons`); `day_secured` при пустом `required` падает в `has_learning_activity`, если юзер graduated **или** заблокирован; тот же допуск в perfect-day (`app/daily_plan/linear/xp.py`); пустой стейт плана называет модуль и незакрытые условия вместо «Завершите настройку ниже» |
| **Optional был заперт при пустом `required`** | Замок «сначала закрой обязательное» гейтился `u_required_settled`, который ложен при `u_required_total == 0`. У graduated и заблокированных единственный доступный контент оставался некликабельным — а `day_secured` требует активности, то есть клика | `u_optional_locked` дополнительно снимается при `u_required_total == 0` |
| **Prereq на модуль без уроков нельзя выполнить** | `_get_module_completion` отдаёт 0% для модуля без уроков, поэтому любая планка выше 0 запирает зависимый модуль навсегда — а через level-entry гейты и весь остаток каталога. Миграция `20260808_level_entry_prerequisites` обходила это выбором цели с уроками; `20260808_normalize_module_prerequisites` и админский импорт — нет | `check_prerequisites` считает prereq на модуль без уроков выполненным (нечего проходить) и пишет warning. Одна точка вместо оговорки в каждом писателе |
| **Ре-импорт модуля возвращал слаги в `prerequisites`** | `CurriculumImportService` писал `data['prerequisites']` дословно; `check_prerequisites` молча выбрасывает строки, то есть первый же ре-импорт снова разгейтил бы модуль (`CNT-014`) | `app/curriculum/prerequisites.normalize_prerequisites(prereqs, level_id)` резолвит `module_<n>` в `modules.id` внутри уровня и проставляет `min_score: 0` / `min_progress: 80`; нераспознанный слаг сохраняется дословно |
| **Sidecar `pairs_<i>` перетирал не-matching ответ** | В `render_final_test_lesson` пары применялись ко всем индексам, пришедшим в форме | Применяются только к вопросам с `type == 'matching'` |
| **Опрос: двойной клик тратил обе попытки** | `POST /api/feedback/survey/dismiss` не гейтился `should_show_survey`, а `SURVEY_MAX_PROMPTS = 2` — два клика подряд закрывали опрос навсегда; неподходящий аккаунт тоже мог сжечь попытку | Оба survey-роута гейтятся `should_show_survey` (409); фронт на 409 показывает терминальное сообщение вместо «попробуйте позже»; `_get_or_create_prompt` больше не может вернуть `None` |
| **Урок без резолвимого модуля снимал блок уровня** | В `find_next_lesson_state` при `module is None` `level_id` становился `None`, не совпадал с `blocked_level_id` и сбрасывал блок; сам осиротевший урок при этом выдавался как следующий | Такой урок пропускается с warning, блок уровня переживает его |

**Правленые тесты (все — про фикстуру, не про предмет теста):** `test_access_control.py` и
`test_curriculum_models.py` строили prereq на модуле без уроков (теперь у него есть урок);
`test_dashboard_ux_audit.py` проверял замок optional при `required=[]` (теперь замок проверяется
на непустом плане, а разблокировка при пустом — отдельным тестом); `test_survey.py` фиксировал
`dismiss_count == 2` после двойного клика (теперь 1 + 409).

**Починен и один pre-existing red вне ветки:** `tests/books/test_reading_session.py` считал дневную
цель чтения от `date.today()` сервера, тогда как продукт считает её от **учебных суток
пользователя** (граница 02:00, фикстурные юзеры в UTC). В окне между 02:00 локального времени и
полуночью UTC базисы расходятся, чередование 5/10 минут даёт другую цель и 10 тестов файла падают.
Базис исправлен на учебные сутки — файл стал детерминированным.

**Гейт:** полный `pytest` — **27 падений, ровно baseline-набор, новых 0** (`comm -23` пусто);
`pytest -m smoke` — 0 падений.
