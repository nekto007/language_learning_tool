...
## Замер (д) — приёмка фазы 2 (Task 7)

Сводка всех «до/после» одной таблицей. Замеры — по копии прода, read-only; детали и SQL — в
секциях (а)–(г) выше.

| Находка | Что меряли | До | После |
|---|---|---|---|
| `DP-087` | начисления `linear_error_review` за день без единой разобранной ошибки | **1 из 4** | 0 — путь закрыт гейтом, три настоящих начисления проходят |
| `DP-042` | закрытий слота `srs:deck_quiz` без квиз-сессии | **1 из 2** (и это единственный закрытый день) | **0 из 5** — слот читает собственный сигнал |
| `DP-050` | XP за grammar-урок на попытке `passed=false` | **0** (прод-инстансов нет) | 0 — приёмка по тест-стражу |
| `DP-005` | curriculum-пунктов в снапшотах, чей урок удалён | **0** из 1888 (инстансов нет) | 0 — замер остался детектором |
| `DP-044` | владельцев deck-quiz-дня, обнуливших колоды | **0 из 5** | 0 — самопочинка на месте |
| `DP-041` | дней с реальной SRS-работой и скрытым каптионом «X из N» | **65 из 69 (94%)** | 0 — числитель перечитывается на каждой отдаче |
| `DP-079`/`DP-103` | комбинаций «эндпоинт × не-объектное тело», отдающих 500 | **20 из 40** | 0 — все 400 по контракту `api_error` |
| `DP-102` | враждебных `?tz=`, роняющих эндпоинт в 500 | 3 длинных имени × 3 GET | 0 — длина режется до `ZoneInfo` |
| `DP-096` | настоящих 500 под JWT, замаскированных под `401` | все | 0 — `try` сужен до разбора токена |
| `DP-098` | сессионных кук, выданных на чистый API-вызов | все успешные JWT-вызовы | 0 — `g._login_user` вместо `login_user()` |

**Три находки закрыты «по механизму».** У `DP-050`, `DP-005` и `DP-044` число «до» равно нулю:
дефект в коде воспроизводится, но в данных путь ещё никто не прошёл. Это записано как результат,
а не как пропуск — приёмка там опирается на тест-страж. Замер `DP-005` остаётся полезным как
**детектор**: ненулевая колонка «урок удалён» означает, что чей-то день не закрывается прямо сейчас.

**Бэкфилла нет ни у одной правки** — политика «XP только вперёд» из фазы 1. Отдельная цена
деплоя у `DP-042`: строк `deck_quiz_completed` в БД ноль, поэтому квиз, пройденный в день выката
до перезапуска, придётся пройти заново; бэкфилл невозможен, `study_sessions` не различает прогон
плана и обычный `/quiz/deck/<id>`.

### Прогоны и откаты

Полностью — секция «Итог фазы 2 (Task 7)» реестра `docs/audit/2026-08-26-daily-plan-audit.md`.
Кратко: полный `pytest` — 10 929 passed / **0 failed** (baseline 10 739 / 0; +190 = ровно новые
тесты фазы); `smoke` 695/695; `ruff` по 14 затронутым файлам **+4**, все `UP045`
(`Optional[X]` под окружающий стиль). Откат каждого кластера даёт красноту:
A-1 10/14, A-2 28/30, B 6/13, C 70/125, D 4/8.
