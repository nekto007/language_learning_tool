...
|---|---|---|---|---|---|
| DP-001 | P1 | `app/api/daily_plan.py:278` | `find_missed_date` режет сутки по календарной полуночи, а `user_today`/`real_activity` — по учебному дню (02:00); клиентский `?tz=` — вторичный вектор | CONFIRMED | код |
