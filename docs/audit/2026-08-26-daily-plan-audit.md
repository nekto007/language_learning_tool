...
`tools/check_consistency.py`, вывод — в секции «Самосогласованность» в конце файла).
...
|---|---|---|---|---|---|
| DP-001 | P1 | `app/api/daily_plan.py:278` | `find_missed_date`/`apply_shield_repair` используют иной tz-базис, чем `user_today`/`real_activity` | CONFIRMED | код |
...
| DP-034 | P1 | `app/curriculum/routes/lessons.py:388` | grammar-уроки платят 9 XP вместо 18: вычищенный ради антифрода `score` доезжает до скейлера как `0.0` | CONFIRMED | код |
...
Ещё три расхождения дал liveness-срез (Task 6) — все со стороной **`CLAUDE.md`**, потому что код
там действительно мёртв:
...
- **Верификация:** CONFIRMED — скептик, настроенный опровергать, опровергнуть не смог. Цитаты:
  ...
  - `app/achievements/streak_service.py:310-316` — внутри `process_streak_on_activity` явно (с комментарием "audit E-003") зашит отдельный `_user_tz = current_user.timezone`, которым считается ТОЛЬКО `real_activity = has_activity_today(user_id, tz=_user_tz)` и `user_today = get_user_local_date(...)` (строка 302).
...
- **Где расхождение:** код | CLAUDE.md | — CLAUDE.md фиксирует только сам факт per-`module_id` кеша («решение кешируется per `module_id`, не дёргается на каждый урок») — это верно и не противоречит кандидату: кандидат тоже утверждает «на каждый модуль», не «на каждый урок». Расхождения нет, но CLAUDE.md умалчивает про отсутствие кросс-запросного кеша и про рост стоимости при наличии `prerequisites`.
...
| DP-044 | P2 | `app/study/game_routes.py:207` | `/study/quiz/linear-plan` без гейта на пустые колоды — required deck-quiz может стать незакрываемым | CONFIRMED | код |
...
| DP-066 | P3 | `app/curriculum/form.py:47` | `anki_cards`/`checkpoint` создаются админ-формой, отсутствуют в `LESSON_TYPE_TO_SOURCE` → незакрываемый required-слот | CONFIRMED | код |
...
- **Симптом:** grammar-уроки платят 9 XP вместо 18: вычищенный ради антифрода `score` доезжает до скейлера как `0.0`
...
#### DP-044 · P2 · `app/study/game_routes.py:207`

- **Кандидат:** `DP-C-070` · линзы-источники: ITEMS-B-08 · скептик: `skeptics/DP-C-070.md`
- **Симптом:** `/study/quiz/linear-plan` без гейта на пустые колоды — required deck-quiz может стать незакрываемым
...
- **Симптом:** `anki_cards`/`checkpoint` создаются админ-формой, отсутствуют в `LESSON_TYPE_TO_SOURCE` → незакрываемый required-слот
...
| DP-078 | P2 | `app/daily_plan/service.py:183` | paused/fallback payload теряет 8 ключей контракта | CONFIRMED | код |
| DP-079 | P2 | `app/api/daily_plan.py:738` | не-dict JSON-тело (`[1]`/`"s"`/`42`) → `AttributeError` → 500 на 8 POST-эндпоинтах из 9 | CONFIRMED | — |
...
| DP-101 | P3 | `app/words/routes.py:1705` | `steps_total=0` при `has_next=true` → прогресс-бар не рендерится у graduated/blocked/paused | CONFIRMED | код |
...
| DP-105 | P3 | `app/api/daily_plan.py:1361` | `LessonSkip` пишется и жрёт квоту, но единственный читатель — мёртвый `chain.py`: план показывает тот же урок | CONFIRMED | код |
| DP-106 | P3 | `app/api/daily_plan.py:748-755` | `/skip-lesson` не гейтится паузой, хотя `slot_skipped` гейтится | CONFIRMED | код |
| DP-107 | P3 | `app/api/daily_plan.py:694-709` | 6 из 9 `event_type` без продюсеров и консьюмеров — csrf-exempt запись без лимита | CONFIRMED | — |
...
- **Симптом:** paused/fallback payload теряет 8 ключей контракта
...
- **Симптом:** не-dict JSON-тело (`[1]`/`"s"`/`42`) → `AttributeError` → 500 на 8 POST-эндпоинтах из 9
...
- **Симптом:** `steps_total=0` при `has_next=true` → прогресс-бар не рендерится у graduated/blocked/paused
...
- **Симптом:** `LessonSkip` пишется и жрёт квоту, но единственный читатель — мёртвый `chain.py`: план показывает тот же урок
...
- **Симптом:** 6 из 9 `event_type` без продюсеров и консьюмеров — csrf-exempt запись без лимита
...
- **Где расхождение:** код | CLAUDE.md | — (инвариант не упомянут в CLAUDE.md — только в собственном комментарии кода, который сам и устарел вместе с удалением `'srs'`-фолбэка).
...
- **Верификация:** CONFIRMED — скептик, настроенный опровергать, опровергнуть не смог. Цитаты:
  ...
  - Поля `data.set_icon`/`data.set_accent`, которые `word_set_quiz.py:51-52` кладёт специально для этого вида карточки, нигде в `app/templates/` не читаются (`grep` — 0 совпадений) — они мертвы на уровне рендера.
- **Где расхождение:** код | CLAUDE.md | — (кандидат не затрагивает задокументированный в CLAUDE.md инвариант; ветка `word-sets-quiz` — незакоммиченная в master фича, в CLAUDE.md не описана)
...
| ID | Sev | Файл:строка | Симптом | Вериф. | Расхождение |
|---|---|---|---|---|---|
| DP-125 | P3 | `tests/daily_plan/test_srs_slot_completion.py:139,168` | два baseline-красных теста сеют карточки по календарной полуночи, а счётчик считает от 02:00 — красное здесь тест, не прод | CONFIRMED | код |
...
#### DP-125 · P3 · `tests/daily_plan/test_srs_slot_completion.py:139,168`
...
- **Сценарий отказа:** по прямому чтению кода и exhaustive grep по `tests/`: 4 эндпоинта (`streak`, `error_review_summary`, `complete_phrase_review`, `streak_repair`) действительно не исполняют ни одной строки тела ни в одном тесте (для `streak` маршрут технически вызывается, но декоратор `api_auth_required` обрывает выполнение до входа в функцию — то есть тело функции не исполняется), и ещё 2 (`complete_error_review`, `challenge_complete`) покрыты только своей самой ранней 400-веткой валидации. Формулировка кандидата (4 нулевых + 2 частичных) подтверждена полностью, включая ненаписанные в якоре два эндпоинта. Оговорка: реальный `pytest --cov` не прогонялся (нерабочее локальное окружение), вывод — из статического чтения кода и текстового поиска по тестам, а не из инструментального coverage-отчёта; это не отменяет вывод, так как grep исчерпывающий (проверены все текстовые варианты путей и прямые импорты функций), но формально это не то же самое, что запущенный coverage.
- **Верификация:** CONFIRMED — скептик, настроенный опровергать, опровергнуть не смог. Цитаты:
...
Прогон приёмки: **2026-08-28**, HEAD `c16244d5`, ветка `word-sets-quiz`. Все четыре критерия
проверены программами, а не глазами; команды и вывод — ниже.

| Критерий | Как проверено | Статус |
|---|---|---|
| Аудит не изменил продакшн-код | `git diff --stat -- app/ tests/daily_plan/ tests/api/ tests/study/` — пусто; `git diff --name-only 0b1f8b7f..HEAD -- app/ tests/` — пусто; 8 коммитов аудита трогают 4 файла, все в `docs/` | ✅ |
| Прогоны воспроизводят baseline | `pytest -m smoke` — совпадает точно; `pytest tests/daily_plan` — на одну красную больше, причина установлена и к аудиту не относится (ниже) | ✅ с оговоркой |
| У каждой находки есть ID, severity, `path:line`, сценарий отказа, вердикт скептика | `tools/check_consistency.py` перепрогнан на Task 10: 127 находок, вердиктов `CONFIRMED` — 127, находок без вердикта — **0** | ✅ |
| Покрыты все 16 эндпоинтов, 44 Python-файла, 3 шаблона, 2 JS — либо финдером, либо явным пропуском | `tools/check_task10_coverage.py` (новый): непокрытых и необъявленных артефактов — **0** | ✅ |
| Числа в шапке = числа в индексе = число секций деталей | `tools/check_consistency.py`, вывод — в секции «Самосогласованность» | ✅ 127 = 127 = 127, расхождений нет |
...
$ git diff --name-only 0b1f8b7f..HEAD -- app/ tests/
(пусто)
$ git diff --stat 0b1f8b7f..HEAD
...
HEAD `0b1f8b7f` до `c16244d5`. Ни один не касается `app/` или `tests/`. Две `M`-строки под `tests/`
...
Сверять надо список имён против `docs/audit/2026-08-08-baseline-pytest.txt`, а третью красную —
против пункта 1–4 выше.

### Критерий 3 — обязательные поля у каждой находки

`tools/check_consistency.py` перепрогнан на Task 10 (read-only, парсит сам реестр). Проверка 4 в
...
Новый скрипт `tools/check_task10_coverage.py`. Ключевое в его методе: «покрыт» засчитывается по
