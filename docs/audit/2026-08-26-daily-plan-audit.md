...
**Что изменилось в коде, одним абзацем.** Введены три канонических хелпера учебного дня —
`study_day_start_utc`, `study_day_bounds_utc` и `study_day_date_for_tz` в `app/utils/time_utils.py`;
`_user_day_boundaries` стала тонкой обёрткой над вторым (сигнатура и политика фолбэка сохранены
дословно). Порядок
...
**Хвосты, дозакрытые код-ревью после приёмки** (тот же класс — второй базис суток в соседнем
потребителе; строки `DP-013` и `DP-051` в таблице выше относятся и к этой правке, не только к
коммитам `e70d30c3`/`a92f267f`): ходоки скилловых серий переведены с календарного `::date` на
`_study_day_date_expr` (иначе `immersion_week` был недостижим для ночного учащегося, пока
`immersion_daily` выдавалась); `compute_ghost_points` сравнивает `race_date` с учебной, а не
календарной датой; `/api/streak/repair` и `/api/streak/repair-web` берут зону из `User.timezone`,
а не из тела запроса; окна учебного дня в `check_immersion_achievement` и
`_try_rollover_from_yesterday` закрываются стартом следующего дня, а не `+24h` (DST); claim-first
у perfect-day снимает заявку при провале начисления; `uq_streak_events_perfect_day` получил
`sqlite_where` рядом с `postgresql_where`; `check_plan_streak_milestone_notification` добавлен на
дашборд шестым подметальщиком. Полный список — в секции «Хвосты, найденные ревью ПОСЛЕ приёмки»
плана `docs/plans/2026-08-29-daily-plan-remediation.md`.
...
**Прогоны на конец фазы (Task 10, приёмка).** `pytest tests/daily_plan -q` (463 теста) и
`pytest -m smoke -q` (695) — **0 падений**. Полный `pytest -q` — **3 падения, все в
`tests/srs/test_counting.py`**, все воспроизведены на дофазовом коммите `aaf25dd8` в отдельном
worktree в те же минуты wall-clock: `app/srs/counting.py` фаза не трогала, а тесты строят фикстуры
от календарной полуночи и краснеют, когда прогон попадает в окно 00:00–02:00 **UTC** (эффективная
зона тестов — UTC, baseline снимался снаружи этого окна, приёмка — внутри). Новых падений,
вызванных фазой, — **0**.

Одно падение фаза всё же принесла и оно **исправлено на приёмке**:
`tests/test_activity_tracker.py::TestStreakShieldOrder::test_shield_repair_counted_by_get_current_streak`
ставил починку на календарное «вчера», а `get_current_streak` после `DP-001` ключуется учебным
днём. Утверждение теста сохранено, на учебный день переехала только арифметика фикстуры.

`ruff check .` — 5143 против 5064 baseline; прод-код дал **−1**, весь прирост (+80) — в новых
русскоязычных тестах и целиком состоит из `RUF002`/`RUF003`/`UP017`, то есть правил, которые
проект нарушает по своей же конвенции. Разбор — в `docs/audit/2026-08-29-remediation-measurements.md`,
секция «(и) Приёмка фазы 1».

Каждый P1 проверен откатом фикса: пять откатов — пять красных стражей (таблица там же).

---

## Статус ремедиации (фаза 2) — 2026-08-29, ветка `daily-plan-remediation-phase-2`

Фаза 2 взяла **17 находок**, сгруппированных по первопричине: кластер A (целостность закрытия
дня и XP) — `DP-037`, `DP-042`, `DP-043`, `DP-046`, `DP-047`, `DP-050`, `DP-087`; кластер B
(заморозка снапшота) — `DP-005`, `DP-041`, `DP-044`; кластер C (контракт API) — `DP-079`,
`DP-081`, `DP-083`, `DP-102`, `DP-103`; кластер D (auth-декоратор) — `DP-096`, `DP-098`.
План — `docs/plans/2026-08-29-daily-plan-remediation-phase-2.md`.

### Ре-верификация на HEAD `b57b43e5` (Task 1)

Якоря `path:line` в реестре сняты на `0b1f8b7f` и сдвинуты фазой 1. Все 17 находок перечитаны
на HEAD; актуальные якоря и механизм каждой — в `docs/audit/2026-08-29-phase2-baseline.txt`,
секция 4. **15 из 17 воспроизводятся.** Baseline прогонов — там же (полный `pytest` зелёный:
10 739 passed / 0 failed, прогон вне окна 00:00–02:00; `smoke` 695/0; `ruff` 5201 + пофайловый
срез по затронутым файлам).

### Отозвано (2026-08-29) — закрыто попутно хвостом код-ревью фазы 1

| ID | Причина отзыва |
|---|---|
| `DP-083` | `/api/streak/repair` (`app/api/daily_plan.py:1243-1271`) больше не читает тело запроса вовсе: зона берётся `get_user_timezone_name(user_id, db.session)` по `DP-001`. Не-строкового `tz` как входа в `_validate_timezone` не существует. Коммит `b57b43e5`. |
| `DP-080` | `/api/streak/repair-web` (`app/words/routes.py:1860-1884`) — та же правка того же коммита, `tz` из `User.timezone`. В объём фазы 2 не входила; проверена по требованию Task 1, потому что чинилась тем же хвостом. |

Обе отозваны **без правки кода** — это результат ре-верификации, а не пропуск. Объём фазы 2
сокращается до **16 находок**.

### Замеры «до» по копии прода (Task 1)

Полностью — `docs/audit/2026-08-29-phase2-measurements.md`. Кратко:

- `DP-087` — **1 из 4** начислений `linear_error_review` в проде выдано в день, когда у
  пользователя не было разобрано ни одной ошибки.
- `DP-042` — **1 из 2** закрытий слота `srs:deck_quiz` получено без квиз-сессии, и это ровно тот
  единственный день с `srs:deck_quiz`, который был закрыт. Деталь сверх реестра: у того
  пользователя за тот день нет **вообще ни одной** `study_sessions` — ключ `linear_srs_global`
  записала корректирующая fallback-ветка `is_srs_slot_completed_today`, вызванная читателем
  слота `srs:global`. Значит правка обязана дать deck-quiz собственный ключ, а не ужесточать
  fallback.
- `DP-050` — **0** подтверждённых случаев: все 19 grammar-прогрессов прода theory-only
  (`score=best_score=last_score=0`), graded grammar-урок с провальной пересдачей ещё не
  встречался. Дефект в коде воспроизводится; приёмка опирается на тест-страж, а не на число.
  Контекст: тот же липкий-статус паттерн в соседних типах уже отработал — 47 попыток с
  `passed=false` лежат на `status='completed'` (dictation 43, final_test 3, translation 1).

### Итог фазы 2 (Task 7) — построчно по всем находкам фазы

**Взято 17: исправлено 16, отозвана 1 (`DP-083` — закрыта хвостом ревью фазы 1). Сверх объёма
проверена и отозвана `DP-080` — та же правка того же коммита. Итого фаза 2 закрывает 18 строк
реестра, открытыми остаются 94 находки** (112 после фазы 1 − 18).

Арифметика сходится с общим итогом: 127 = 15 (фаза 1) + 18 (фаза 2) + 94 открытых. В разделе
«Post-Completion» плана фазы 2 стоит 95 — та оценка считала `DP-080` внутри взятых семнадцати,
хотя находка была проверена сверх объёма; верное число — 94.

| ID | Sev | Кластер | Статус | Коммит | Тест-страж |
|---|---|---|---|---|---|
| `DP-087` | P2 | A-1 | ✅ | `a93c5b2c` | `tests/daily_plan/test_day_close_integrity.py::TestErrorReviewRequiresProofOfWork` (6) |
| `DP-042` | P2 | A-1 | ✅ | `a93c5b2c` | `…::TestDeckQuizHasOwnCompletionSignal` (4) |
| `DP-050` | P2 | A-1 | ✅ | `a93c5b2c` | `…::TestGrammarRetakeXpGate` (4, параметризован по обоим URL) |
| `DP-037` | P2 | A-2 | ✅ | `ee192b20` | `tests/daily_plan/test_vanishing_slots.py::TestAcuteErrorReviewStaysInThePlan` |
| `DP-043` | P2 | A-2 | ✅ | `ee192b20` | `…::TestCollapseTierKeepsReviewBatch` |
| `DP-046` | P2 | A-2 | ✅ | `ee192b20` | `…::TestPhraseReviewGraderRejectsEmptyAnswers` |
| `DP-047` | P2 | A-2 | ✅ | `ee192b20` | `…::TestPhraseNormalisationKeepsPhrasesDistinct` |
| `DP-005` | P2 | B | ✅ | `be376f60` | `tests/daily_plan/test_snapshot_v2.py::TestDeletedLessonSelfHeal` (5) |
| `DP-044` | P2 | B | ✅ | `be376f60` | `…::TestDeckQuizWordsGoneSelfHeal` (4) |
| `DP-041` | P2 | B | ✅ | `be376f60` | `…::TestFrozenSrsCountersAreRefreshed` (4) |
| `DP-079` | P2 | C | ✅ | `fd7a65be` | `tests/api/test_daily_plan_body_contract.py::TestNonObjectBodyIsRejected` (40) |
| `DP-081` | P2 | C | ✅ | `fd7a65be` | `…::TestEventsFieldConfusions` |
| `DP-103` | P3 | C | ✅ | `fd7a65be` | `…::TestBrokenJsonIsNamedAsSuch` + `TestMissingBodyStillBehavesAsEmptyObject` |
| `DP-102` | P2 | C | ✅ | `fd7a65be` | `…::TestTimezoneParamNeverCrashes` (18 = 6 враждебных `tz` × 3 GET) |
| `DP-096` | P2 | D | ✅ | `3bc812a2` | `tests/api/test_jwt_decorator_contract.py` — `test_handler_exception_under_jwt_is_not_masked_as_401`, `…_becomes_500_over_http`, `test_non_numeric_identity_returns_401_not_500` |
| `DP-098` | P2 | D | ✅ | `3bc812a2` | `…::test_successful_jwt_call_sets_no_session_cookie`, `…_leaves_no_session_for_next_request` |
| `DP-083` | P3 | C | ↩︎ **отозвана** | `b57b43e5` (фаза 1) | — (см. таблицу отзыва выше) |
| `DP-080` | P3 | — | ↩︎ **отозвана** | `b57b43e5` (фаза 1) | — (в объём фазы 2 не входила, проверена по требованию Task 1) |

**Что изменилось в коде, одним абзацем.** У deck-quiz появился собственный сигнал завершения
`DailyPlanEvent('deck_quiz_completed')` — слот больше не читает общий `linear_srs_global`, который
пишет и обычная `/study`-сессия, и корректирующий fallback. `/error-review/complete` требует
доказательства работы (свои строки, разобранные внутри текущего **учебного** дня), гейт XP
grammar-урока переведён со липкого `status` на результат этой попытки. Острый `error_review`
перестал исчезать из плана (поднимается во главу optional), тир `collapse` больше не обнуляет
партию ревью (`RECOVERY_REVIEW_FLOOR`), пустой ответ в «Повтори фразы» не засчитывается верным, а
ключ дедупа фраз различает фразы, а не шаблоны. `overlay_completion` получил общий предикат
`_item_unreachable` (чтение из фазы 1 + удалённый урок + пропавшие колоды) и живой числитель SRS.
Все шесть POST-эндпоинтов зоны разбирают тело одним `_json_object_body()`, `_validate_timezone`
режет враждебный `?tz=` до обращения к файловой системе. В `api_auth_required` `try` сужен до
разбора токена, а сессионная кука на JWT-вызов больше не выдаётся.

**Три находки закрыты «по механизму», а не по числу.** У `DP-050`, `DP-005` и `DP-044` замер «до»
дал **0 прод-инстансов**: дефект в коде воспроизводится, но пути в данных пока никто не прошёл.
Это записано как результат, а не как пропуск — приёмка там опирается на тест-страж. Замер `DP-005`
одновременно служит детектором: ненулевая колонка «урок удалён» означает, что чей-то день не
закрывается прямо сейчас.

**Находки сверх реестра, найденные по ходу.** (1) Обработчиков grammar-урока **два** —
`render_grammar_lesson` (диспетчер `/learn/<id>/`) и роут `grammar_lesson`
(`/curriculum/lesson/<id>/grammar`); разошедшиеся близнецы того же рода, что `CNT-001`. Гейт
`DP-050` исправлен в обеих копиях, но **сведение их в одну в объём фазы 2 не входило** — это
открытый хвост. (2) Копия сломанной регулярки `DP-047` жила и в `app/api/daily_plan.py`; она
удалена, а не починена — `_normalise_phrase_answer` делегирует в канонический `normalise_phrase`.
(3) Call-site'ов `_validate_timezone` три, а не шесть, как записано в теле `DP-102`.

**Что осталось открытым и почему.**

- **94 находки** реестра. Крупные группы: UX-очередь optional (`DP-036`/`DP-038`/`DP-039`),
  N+1 и стоимость сборки (`DP-017`…`DP-021`, `DP-028`…`DP-032`), мёртвый код (`DP-119`…`DP-124`).
- `DP-095` (18 роутов зоны отдают план без `@module_required('words')`) — **не правка, а политика
  доступа**: нужно продуктовое решение владельца, гейтить ли план дня модулем вообще.
- `DP-011` (смена `User.timezone` переигрывает идемпотентные ключи) — отложена ещё в фазе 1,
  причина в теле находки.
- **3 PLAUSIBLE** (пауза против `/next-slot`, гонка двойной починки серии,
  `REMEMBER_COOKIE_SAMESITE` в браузере) ждут живого стенда.
- Хвост фазы 2: два обработчика grammar-урока (см. выше).
- **Бэкфилла нет ни у одной правки** — политика «XP только вперёд» из фазы 1 действует и здесь.
  Отдельная цена деплоя: строк `deck_quiz_completed` в БД ноль, поэтому квиз, пройденный в день
  выката до перезапуска, придётся пройти заново (бэкфилл невозможен — `study_sessions` не
  различает прогон плана и обычный `/quiz/deck/<id>`).

**Прогоны на приёмке (Task 7).**

| Прогон | Baseline (Task 1) | Приёмка | Вердикт |
|---|---|---|---|
| полный `pytest -q -p no:randomly` | 10 739 passed / **0 failed** | 10 929 passed / **0 failed** (69 skipped, 5 xpassed, 4 xfailed; exit 0) | список FAILED пуст с обеих сторон, сверять по именам нечего; +190 = ровно новые тесты фазы (14+30+13+125+8) |
| `pytest -m smoke -q` | 695 / 0 | 695 / 0, exit 0 | без изменений |
| `ruff` по затронутым файлам | см. пофайловый срез baseline | **+4** на 14 файлах | весь прирост — `UP045` (`Optional[X]`), см. ниже |

Каждый кластер проверен **откатом фикса**: `app/` откачен на `b57b43e5` в отдельном worktree,
тесты фазы 2 оставлены на месте.

| Кластер | Способ отката | Красных | Записано по ходу |
|---|---|---|---|
| A-1 | весь `app/` на `b57b43e5` | **10** из 14 | 10 — совпало |
| A-2 | `git revert -n ee192b20` | **28** из 30 | 28 — совпало |
| B | `git revert -n be376f60` | **6** из 13 | 6 — совпало |
| C | `git revert -n fd7a65be` | **70** из 125 | 43 — **не воспроизвелось** |
| D | `git revert -n 3bc812a2` | **4** из 8 | 4 — совпало |

Метод: worktree на HEAD, `git revert -n <коммит кластера>`, затем `git checkout HEAD -- tests/`
(правка откачена, стражи оставлены). Гейт «минимум один красный тест на кластер» выполнен во
всех пяти с запасом.

Две оговорки, чтобы следующий читатель не потерял время:

1. **Откат A-1 отдельным `revert` не проходит** — конфликт: `a93c5b2c`, `ee192b20` и `fd7a65be`
   правят одни и те же строки `app/api/daily_plan.py`. Поэтому A-1 мерян откатом всего `app/`.
   По той же причине пофайловый `git checkout b57b43e5 -- app/api/daily_plan.py` не изолирует ни
   один кластер: файл несёт хунки трёх.
2. **Цифра 43 для кластера C, записанная по ходу Task 5, не воспроизводится** — точный откат
   коммита даёт 70 красных из 125 (10 уникальных тест-функций до раскрытия параметризации).
   Чем именно мерили тогда, восстановить не удалось; здесь приведено число, которое
   воспроизводится указанным выше методом. На вердикт это не влияет: 70 > 0.

**`ruff` — гейт «не выросло по затронутым файлам», замер откатным worktree.** Фаза 2 тронула 14
файлов `app/`; суммарно **+4** нарушения, все — `UP045` (`Optional[X]` вместо `X | None`):
`app/srs/counting.py` 13→16, `app/daily_plan/items/error_review.py` 8→9. Это правило проект
нарушает по своей же конвенции — в тех же файлах `Optional[...]` уже стоит 11 и 8 раз
соответственно, и новые сигнатуры написаны под окружающий стиль (та же логика, по которой в фазе 1
принят прирост `RUF002`/`RUF003`/`UP017`). Единственное нарушение вне этого класса — `RUF022`
(`__all__` не отсортирован) в `app/daily_plan/items/phrase_review.py`, появившееся от добавления
`normalise_phrase` — **исправлено на приёмке**, файл вернулся к baseline 5. Остальные 12 файлов —
без изменений; `app/api/daily_plan.py` 5 = 5 при +207 строк диффа.
...
- **Второй механизм того же симптома (найден на код-ревью 2026-08-28, не отдельная находка — тот же P1, тот же замер):** расхождение базисов объясняет, почему щит уходит на *неверную* дату; порядок починок в `process_streak_on_activity` объясняет, почему он уходит **вместо бесплатной**. Внутри одного вызова щит стоит первым (`app/achievements/streak_service.py:358-390`, гейт — только `real_activity`, без `steps_done`), бесплатная починка — вторым (`:393-396`), бесплатный авто-хилер — третьим (`:404-410`). `apply_shield_repair` пишет `StreakEvent(event_type='shield_repair')`, а `has_repair_for_date` (`:774-780`) считает его починкой наравне с `free_repair`; `find_auto_heal_date` пропускает любую дату, по которой починка уже есть (`:940-941`). Значит на дырке, которую авто-хилер закрыл бы бесплатно, щит списывается раньше и делает её для хилера невидимой. Перекрытие полное на offset 1–3: `find_missed_date` возвращает **первую** незалатанную дырку с якорем в прошлом (`:856-893`, `max_days=7`), а `find_auto_heal_date` (`max_days=3`) требует якорь ещё и в будущем — на offset=1 это `today_has_activity` (`:944-945`), что уже гарантировано гейтом `real_activity`, а на offset 2–3 якорь даёт день, который `find_missed_date` перед этим пропустил как активный. Щит незаменим только на offset 4–7, куда авто-хилер не дотягивается. Ремедиации DP-001 недостаточно свести базисы суток: без переноса щита ЗА `auto_heal_streak_on_activity` (или без проверки `find_auto_heal_date` перед списанием) замеренные «6 из 11 щитов» уменьшатся, но не обнулятся.
...
- **Решение ремедиации (фаза 1, 2026-08-29, Task 8): ОСТАЁТСЯ ОТКРЫТОЙ, сознательно.**
  Малого средства нет. Идемпотентность держится не на прикладной проверке, а на
  уникальном индексе `uq_streak_events_xp_linear_source (user_id, event_date, details->>'source')`
  (`migrations/versions/20260621_idempotency_constraints.py`) — то есть буквально на
  значении колонки `event_date`. Записать исходную зону в `StreakEvent.details` дёшево,
  но бесполезно: `_already_awarded` сравнивает `event_date == when`, где `when`
  пересчитан «сейчас», и чтобы сделать сравнение tz-устойчивым, надо перенести дедуп
  с даты на «дата + зона на момент записи» — новая колонка, новый уникальный индекс,
  бэкфилл всех исторических строк и синхронная правка всех write-path'ов
  (`xp_linear`, `xp_curriculum_lesson`, `DailyPlanLog`, `DailyPlanEvent`,
  `DailyStudyMinutes`, route-steps). Это отдельная задача со своей миграцией, а не
  правка внутри кластера границы суток. Триггер при этом остаётся редким и
  добровольным (смена таймзоны в `/profile`), а выгодоприобретатель двойного
  начисления — сам инициатор. Кластер границы суток (`DP-001`…`DP-027`) на эту
  находку не влияет: он приводит все базисы к одному учебному дню, но ни один из них
  не привязывает дату к историческому значению `User.timezone`.
...
| DP-113 | P3 | `app/templates/partials/unified_daily_plan.html:46-69` | 4 словаря kind'ов, ни один не совпадает с фактическим набором; `word_set_quiz` не описан нигде (лейбл и глиф закрыты на ветке — см. тело находки; мёртвые `data.set_icon`/`set_accent` открыты) | CONFIRMED | код |
...
- **Статус на HEAD ветки (дописано 2026-08-28).** Находка зафиксирована на базисе аудита `0b1f8b7f`,
  и цитаты выше верны **для него**. После приёмки код-ревью фичи наборов слов закрыло бо́льшую половину:
  `afd24dc8` добавил `'word_set_quiz': 'Тематический квиз'` в `kind_section_labels`
  (`app/templates/partials/unified_daily_plan.html:55`) и `'word_set_quiz': '🧠'` в `kind_icons` (`:70`),
  поэтому карточка теперь рендерится с eyebrow-лейблом и глифом, а цитата «0 упоминаний в
  `kind_section_labels`, `kind_icons`» на HEAD **не воспроизводится**. **Открытым остаётся хвост:**
  `data.set_icon`/`data.set_accent`, которые `items/word_set_quiz.py:61-62` кладёт в payload,
  по-прежнему не читаются ни одним шаблоном (`grep` по `app/templates/` — 0 совпадений), то есть
  либо шаблон должен их использовать, либо builder — перестать их класть. Прецедент аннотации — DP-033.
...
- **Статус на HEAD ветки (2026-08-28):** исправлено вне диапазона аудита. `3fd29d85` (код-ревью фичи наборов слов) заменило календарный якорь на `day_to_naive_utc` в обоих тестах; `pytest tests/daily_plan` на HEAD даёт 0 падений. Находка остаётся в реестре как описание механизма — сам аудит её не чинил.
...
|---|---|---|
| Аудит не изменил продакшн-код | `git diff --name-only 0b1f8b7f..967504d6 -- app/ tests/` — пусто; 10 коммитов аудита трогают 4 файла, все в `docs/`. **Диапазон закрытый, не `HEAD`:** после приёмки на ветку легли `137e98bb`/`6b0a5ba2`/`d2ce19aa`/`bf78fdcb`/`afd24dc8` (код-ревью фичи наборов слов, к аудиту не относятся), и они `app/`/`tests/` трогают | ✅ |
...
$ git diff --stat 0b1f8b7f..967504d6
 ...
 docs/audit/2026-08-26-daily-plan-audit.md     | 2779 +++++++++++++++
 docs/plans/2026-08-26-daily-plan-audit.md     |  172 +
 4 files changed, 3265 insertions(+)
```

Проверка сильнее, чем «diff по рабочей копии пуст»: сверены **все 10 коммитов аудита** от базисного
...
приёмки в ту же ветку легли правки код-ревью фичи наборов слов (`137e98bb`, `6b0a5ba2`, `d2ce19aa`,
`bf78fdcb`, `afd24dc8`), которые трогают `app/` и `tests/` — в том числе `tests/daily_plan/test_events.py`,
`app/api/books_catalog.py` (короткий репро DP-033) и `app/templates/partials/unified_daily_plan.html`
(лейбл/глиф `word_set_quiz`, половина DP-113). Обе затронутые находки аннотированы в своих телах. Поэтому та же команда с `HEAD` сегодня
...
для будущих прогонов он снят.

**Статус на HEAD ветки (2026-08-28).** Ожидание «2 унаследованные красные» тоже больше не
выполняется: `3fd29d85` применило к `tests/daily_plan/test_srs_slot_completion.py` предписание
самой же DP-125 (`_study_day_start` → `day_to_naive_utc`). Прогон `pytest tests/daily_plan` на HEAD
даёт **0 падений**. Обе цифры выше — исторические, относятся к диапазону аудита
`0b1f8b7f..967504d6`; сверять будущие прогоны надо с нулём, а не с двойкой.
