...
- [x] Добавить паттерн `vocab_pull` в CLAUDE.md (раздел Key Patterns): `extract_chapter_vocab` + `queue_vocab_as_srs` в `app/books/vocab_pull.py`, source='book_reading', fired on `daily_target_met` transition
