...
- [ ] Определить `def queue_vocab_as_srs(words, user_id, db_session) -> int`:
...
- [ ] Написать unit-тесты в `tests/books/test_vocab_pull.py`: extract вернул правильные слова, stop words фильтруются, слова уже в UserWord пропускаются, `next_review >= tomorrow`
- [ ] Запустить `pytest tests/books/test_vocab_pull.py` — должны пройти
...
- [ ] В `reading_session_end()` перед вызовом `end_session()` сохранить `was_target_met = is_daily_reading_target_met_today(current_user.id, chapter.book_id, db)` (запрос только по closed sessions — safe)
- [ ] После вычисления `state` и `daily_target_met_today` добавить: если `not was_target_met and daily_target_met_today` — вызвать `extract_chapter_vocab()` используя `state['earliest_start_offset']` и `state['current_offset']`, затем `queue_vocab_as_srs()`, сохранить `queued_vocab_count`; ошибки обёртываем в `try/except` (best-effort, не блокируем response)
- [ ] Добавить `queued_vocab_count` в response dict (0 если не запускался или ошибка)
- [ ] Запустить `pytest -m smoke` — должны пройти

### Task 4: Verify acceptance criteria

- [ ] Запустить полный тест-сьют: `pytest`
- [ ] Проверить импорты: `python -c "from app.books.vocab_pull import extract_chapter_vocab, queue_vocab_as_srs"`
- [ ] Убедиться: coverage на `vocab_pull.py` ≥ 80%
