...
- [x] `pytest tests/daily_plan -q` и `pytest -m smoke -q` — **0 падений**
      → 463 и 695 тестов, exit 0 у обоих
- [x] полный `pytest -q` — список FAILED **не длиннее** baseline Task 1; сверка по именам, а не по числу
      → **3 FAILED против 1 в baseline, но новых падений от фазы 0.** Baseline снимался в 22:45 UTC,
      приёмка — в 00:30–01:30 UTC, то есть внутри окна 00:00–02:00, о котором baseline и
      предупреждает (эффективная зона тестов — UTC). Все три имени — `tests/srs/test_counting.py`,
      файл `app/srs/counting.py` фазой не тронут, и все три воспроизведены на дофазовом `aaf25dd8`
      в отдельном worktree в те же минуты. Четвёртое падение того же прогона
      (`test_shield_repair_counted_by_get_current_streak`) дофазовый код проходил — оно и было
      единственным следствием фазы, **исправлено здесь же**: фикстура переехала с календарной
      полуночи на учебный день, утверждение теста не изменилось
- [x] `ruff check .` ≤ baseline Task 1
      → **5143 против 5064, порог не выдержан осознанно.** Прод-код дал −1 (3476 против 3477), весь
      прирост +80 — в 1930 строках новых тестов и целиком состоит из `RUF002`/`RUF003`
      («неоднозначный юникод» на кириллице) и одного `UP017` (`timezone.utc`, как во всём проекте).
      Замечаний классов `F`/`E`/`B`/`SIM` в новых файлах нет. Порог недостижим буквально, пока
      тесты пишутся по-русски: `select = ["RUF"]` включает эти правила оптом, и они же составляют
      основную массу самих 5064. Глушить их правкой `pyproject.toml` на приёмке не стали — это
      отдельное решение владельца
- [x] у каждой из 16 находок есть тест-страж, краснеющий при откате фикса — проверить выборочным
      откатом минимум по одному тесту на каждый P1
      → пять откатов, пять красных: `DP-001` базис (6 тестов), `DP-001` порядок починок (3),
      `DP-033` (4), `DP-034` (3), `DP-035` (1). Таблица — в измерениях, секция «(и) Приёмка фазы 1»
- [x] `docs/audit/2026-08-29-remediation-measurements.md` содержит пары «до/после» по трём
      прод-замерам Task 1 и по расхождению `streak` из глобального перехода на 02:00
      → секции (а)/(б)/(в) и (д); плюс добавлена секция «(и) Приёмка фазы 1» с прогонами,
      разбором каждого FAILED и матрицей откатов
- [x] `git log --oneline` фазы: коммиты по находкам, ни одного упоминания AI/Claude
      → семь коммитов: baseline, пять `fix(daily-plan): …` по находкам, один `feat:` с
      документацией. Сообщение документарного коммита переписано — из тела убрано имя
      gitignored-файла инструкций, чтобы в истории не осталось имени инструмента
...
## Хвосты, найденные ревью ПОСЛЕ приёмки (закрыты)

Приёмка Task 10 замерялась на дереве, которое затем ещё двигалось. Ниже — что нашло
код-ревью фазы и что с этим сделано. Все пункты — того же класса, что чинила фаза:
второй базис суток, доживший до прода в соседнем потребителе.

- [x] **Ходоки скилловых серий остались на календарном `::date`.** `check_immersion_achievement`
      переехал на учебный день (`DP-013`), а `get_immersion_streak`, который ОН ЖЕ зовёт ради
      `immersion_week`, — нет. Раскладка «слушание в 23:00 + три навыка в 01:00» — один учебный
      день и две календарные даты: дневная ачивка выдавалась, недельная не могла выдаться никогда.
      Введён `_study_day_date_expr` (SQL-двойник `study_day_date_for_tz`), на него переведены все
      четыре ходока (listening / writing / speaking / immersion). Приёмка ревью нашла, что
      исключение в этой строке называло несуществующий символ `get_activity_calendar`: реально
      календарным оставался `get_streak_calendar` (публичная `/streak/<username>`), а он отдаёт
      `current_streak`/`longest_streak` и на краю окна передаёт эстафету `get_current_streak` —
      то есть смешивал два базиса в одном ответе. Переведён на `study_day_date_for_tz` +
      `_study_day_date_expr` тем же ревью
- [x] **Очки призраков дневной гонки сравнивались с календарной датой.** `DP-012` сделал
      `race_date` учебной датой на обеих поверхностях, но `compute_ghost_points` держал
      `now.date()`: в окне 00:00–02:00 каждый призрак допрыгивал до полной цели, пока гонка идёт
- [x] **Платная починка серии выбирала чинимую дату по клиентскому `tz`.** `/api/streak/repair`
      и `/api/streak/repair-web` брали зону из тела (и `DEFAULT_TZ` при её отсутствии), а
      `find_missed_date` / `has_repair_for_date` / `get_current_streak` / `auto_heal_streak_on_activity`
      после фазы читают учебный день `User.timezone`. Монеты уходили бы на `spent_repair` под датой,
      которую ни один читатель не считает дырой. Обе ручки переведены на `get_user_timezone_name`
- [x] **Окна учебного дня закрывались фиксированными `+24h`.** `check_immersion_achievement` и
      `_try_rollover_from_yesterday` считали `day_end = day_start + timedelta(days=1)`, хотя
      учебный день в день перевода стрелок длится 23 или 25 часов. Оба конца теперь идут через
      `study_day_start_utc` / `_local_date_start_naive_utc`
- [x] **`check_plan_streak_milestone_notification` жил только на `/api/daily-status`** — той
      поверхности, про которую код фазы сам пишет «no client actually calls». Тот же класс, что
      `DP-035`: подметальщик добавлен на дашборд, ПОСЛЕ `process_streak_on_activity` (ему нужен
      посчитанный streak). `_secured_date` поднят выше `try`, чтобы упавший подметальщик не уносил
      дату у идущих следом
- [x] **`uq_streak_events_perfect_day` был партиальным только на PostgreSQL.** `postgresql_where` —
      диалектный kwarg, остальные диалекты его молча роняют, и на sqlite-фолбэке `DATABASE_URL`
      индекс становился UNIQUE по ВСЕМ строкам `streak_events` за user+date. Добавлен `sqlite_where`
- [x] **Тесты, переставшие проверять то, что заявляют.** Гейт `DP-033` (книга без прав/модуля)
      срабатывал раньше ветки, ради которой тест писался: три теста чтения получили
      `rights_status='public_domain'`. `test_no_award_when_plan_paused` строил план с пустым
      `required` и `graduated=False` — его отклоняло правило допуска, а не проверка паузы
      (`graduated=True`). Дедуп майлстоуна считал уведомления без фильтра по типу и проходил при
      нуле созданных. Ни один тест не покрывал `_session_pk` — добавлен параметризованный набор по
      трём эндпоинтам плюс явная проверка, что `True` не резолвится в сессию №1
