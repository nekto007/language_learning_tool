...
- [x] `git diff --stat` по `app/` и `tests/` — пусто (аудит не правит код; любое изменение здесь = нарушение задания)
- [x] `pytest tests/daily_plan -q` и `pytest -m smoke -q` дают тот же результат, что baseline из Task 1 (аудит не мог ничего сломать, это проверка гигиены прогонов)
- [x] каждая находка реестра имеет: ID, severity, `path:line`, сценарий отказа, вердикт скептика; находок без вердикта — ноль
- [x] все 16 эндпоинтов, все 44 Python-файла зоны, 3 шаблона и 2 JS либо покрыты финдером, либо перечислены в «сознательных пропусках» с причиной

### Task 11: Update documentation

- [x] добавить в `CLAUDE.md` короткую ссылку на новый реестр в блоке аудитов (без переписывания раздела Daily Plan — исправления инвариантов делает ремедиация, не аудит)
- [x] если аудит нашёл расхождения именно в тексте `CLAUDE.md` — перечислить их в реестре как отдельный список с предлагаемыми формулировками, но **не править** документ в рамках аудита
- [x] README не трогать — пользовательских изменений нет
