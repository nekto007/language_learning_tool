...
- [x] `complete_lesson(user_id, lesson_id)` вызывает `award_curriculum_lesson_xp_idempotent(user_id, lesson_id, today_date, db)`
- [x] Идемпотентность: `StreakEvent.query.filter_by(user_id, event_type='xp_curriculum_lesson').filter(details.lesson_id == lesson_id, event_date == today)` → если есть, skip
- [x] При создании новой StreakEvent — `db.session.flush()` не `commit()` (caller commits)
- [x] Write tests: первый вызов complete_lesson → XP awarded; второй вызов в тот же день → XP не дублируется; в новый день → XP снова awarded
...
- [x] `pytest -m smoke` — все green
- [x] `pytest tests/study/` — all pass (suite lives at tests/test_study_*.py — 407 passed)
- [x] `pytest tests/achievements/` — all pass
- [x] `pytest tests/curriculum/` — all pass
- [x] `pytest tests/admin/` — all pass
- [x] Manual: зайти как тестовый user, завершить карточную сессию → `UserStatistics.total_xp` увеличился, уровень на дашборде обновился (skipped — not automatable; covered by tests/test_study_api_routes.py::TestCompleteSession::test_awards_xp_on_completion + Task 2 unification tests)
- [x] Manual: завершить карточную сессию → лидерборд отражает новый XP (skipped — not automatable; covered by Task 3 leaderboard tests in tests/test_stats_service.py)
- [x] Manual: получить достижение дважды быстро (через API) → в БД только одна запись (skipped — not automatable; covered by Task 6 race-condition test)
