...
- [x] Добавить `data-completion-mode="plan"|"standalone"` на `#lesson-completion` для тестов и CSS-хуков
- [x] Write tests: план-контекст активен + не последний slot → «Следующий слот плана» CTA; плана нет → старое поведение; secured → redirect
...
- [x] Найти все inline-ссылки типа `href="/learn/{{ next_lesson.id }}"` вне `#lesson-completion` (другие кнопки/ссылки)
- [x] Если контекст плана активен — скрывать их CSS-ом через `[data-completion-mode="plan"] ~ .legacy-next-link { display: none }` или через JS-тоггл в `linear-plan-context.js`
- [x] Quiz.html: если completion-блок теперь вызывается (мы его раньше не вызывали) — добавить вызов `showLessonCompletion` аналогично другим типам; иначе — только footer-кнопка, но с plan-aware логикой в JS
- [x] Write tests: при plan-активном контексте в rendered HTML нет dangling-ссылок на curriculum-next кроме CTA внутри completion-блока
...
- [x] В dashboard route: `show_day_secured_banner = request.args.get('day_secured') == '1' and DailyPlanLog for today has secured_at`
- [x] В partial: если `show_day_secured_banner` → рендерим блок с:
...
- [x] JS: auto-clear `linearPlanContext` при наличии баннера (для страховки)
- [x] Write tests: баннер рендерится при query-param; XP-сумма корректно агрегируется из сегодняшних awards; без query — баннер отсутствует
