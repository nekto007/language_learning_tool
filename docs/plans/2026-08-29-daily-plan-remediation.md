...
- [x] внести в `CLAUDE.md` семь документарных расхождений `CM-1…CM-7` по формулировкам, уже
...
- [x] внести `CM-8…CM-10`: «mission/linear-chain **отключены**, а не удалены» — 2 615 строк
      физически на месте и удерживаются тестами; и `CM-11` — константа называется
      `DAILY_LESSON_SKIP_QUOTA`, а не `DAILY_SKIP_QUOTA`
- [x] обновить раздел Daily Plan под фактически внесённые изменения: единый базис 02:00 во всех
      окнах активности, гейт доступа в required-слоте чтения, perfect-day как подметальщик на
      обоих писателях `secured_at`
- [x] в реестре аудита завести секцию «Статус ремедиации (фаза 1)»: по каждой из 16 находок —
      ✅/🟡/отложено, коммит и тест-страж; для `DP-034`/`DP-035` явно записать «история не
      добирается — решение владельца»
      → секция «Статус ремедиации (фаза 1)» вставлена перед «Расхождения с `CLAUDE.md`»:
      таблица 16 находок (15 ✅ / 1 🟡 `DP-011`), у каждой закрытой — коммит и класс-страж,
      плюс сквозной `TestOneTodayAcrossSurfaces`; оба решения владельца («глобальные 02:00»,
      «XP только вперёд») записаны в преамбулу секции. В преамбуле «Расхождения с `CLAUDE.md`»
      снята формулировка «не внесённая правка» — `CM-1…CM-11` применены.
      **`CLAUDE.md` в `.gitignore` и не отслеживается** (`git check-ignore` → строка 205),
      поэтому правки документа в коммит не входят и в другие checkout'ы git'ом не доедут —
      это то же ограничение, что уже описано в самом файле для контент-фикса `E-092`.
