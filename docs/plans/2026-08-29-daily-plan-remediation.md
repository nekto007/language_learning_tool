...
- [ ] написать падающие тесты **до** правки: активность в 00:30 локального времени принадлежит
...
- [ ] ввести **один** канонический хелпер окна учебного дня в `app/utils/time_utils.py` —
...
- [ ] `_user_day_boundaries` в `app/telegram/queries.py:22` сделать **тонкой обёрткой** над новым
...
- [ ] в `process_streak_on_activity` (`app/achievements/streak_service.py:282`) свести все три базиса
...
- [ ] переписать комментарий `streak_service.py:310-313`: в нём **ложное** утверждение «Activity
...
- [ ] прогнать телеграм-тесты отдельно (`pytest tests/telegram -q`): у планировщика четыре гейта
...
- [ ] записать замер «после» по расхождению `streak` из Task 1 в файл измерений
...
- [ ] тест «до»: дырка на offset 1–3, у юзера активен щит **и** доступен бесплатный авто-хилер →
      сейчас списывается щит; после фикса щит остаётся, дырка закрывается бесплатно
- [ ] переставить щит **за** `auto_heal_streak_on_activity` (`:404-410`) либо проверять
...
- [ ] проверить, что `apply_shield_repair` по-прежнему срабатывает на offset 4–7, куда авто-хилер
      (`max_days=3`) не дотягивается — щит не должен стать недостижимым
- [ ] замер «после»: доля щитов, списанных на днях с активностью, по копии прода (реестр: 6 из 11)
...
- [ ] тесты «до», три состояния: `licensed`-книга с истёкшим `expiration_date`; снятый у юзера
...
- [ ] `_book_is_actionable_for_reading` (`app/daily_plan/items/reading.py:50`) дополнить
...
- [ ] **отдельно — заморозка снапшота.** Гейт в билдере закрывает только новый день; книга, к
...
- [ ] закрыть подслучай, который реестр отдельно называет тянущим на P0: у юзера без модуля `books`
      ...
      а не незакрываемый
- [ ] тест на самопочинку: слот в снапшоте + доступ отозван в середине дня → `day_secured`
      достижим по остальным required
- [ ] замер «после»: у юзера из замера (а) Task 1 слот чтения больше не появляется в required
