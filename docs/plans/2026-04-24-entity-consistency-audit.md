...
- [ ] Проверить существующую реализацию `_count_active_users_in_range()` — добавить `UserLessonProgress` как 7-й источник в UNION (или удостовериться что StudySession уже покрывает book-course активность)
- [ ] `_activity_date_expr(last_activity, completed_at)` — убедиться что COALESCE-логика даёт правильный день; если `completed_at` в другой TZ чем `last_activity` → возможна ошибка определения дня
- [ ] Write tests: пользователь с только `UserLessonProgress.completed_at` сегодня → считается в DAU
...
- [ ] Найти функцию которая создаёт `UserAchievement` — grep `UserAchievement(` по app/
- [ ] Обернуть вставку в `try/except IntegrityError`: если raised — rollback savepoint, вернуть existing `UserAchievement`; не бросать 500
- [ ] Добавить `select_for_update=False` hint или просто обработку IntegrityError (INSERT ... ON CONFLICT DO NOTHING не всегда доступен через ORM без raw SQL)
- [ ] Проверить что `notify_in_app_achievements` проверяется ДО создания notification (не после)
- [ ] Write tests: двойной вызов `grant_achievement` для одного user+achievement → один ряд в БД, нет IntegrityError в ответе
