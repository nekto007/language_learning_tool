...
- [ ] `/api/daily-status` и `/api/daily-plan`: пересобирать цепочку каждый запрос (chain само-наращивается по факту активности, плюс сервер пересчитывает completed по `plan_completion`)
- [ ] вернуть в payload `poll_after_seconds` (или просто полагаться на существующий polling в JS) — полная пересборка ответа уже даёт обновление
- [ ] добавить тест: после маркера активности новая цепочка длиннее на 1; вторая активность ещё на 1; источник исчерпан → длина не растёт
- [ ] pytest зелёный
...
- [ ] показать индикатор "+1 задание добавлено" при увеличении длины цепочки между двумя ответами `/api/daily-status` (JS сравнивает длину, показывает короткий toast)
- [ ] над baseline-частью цепочки — sticky-метка "Минимум на день" (первые N), под ней — "Дальше необязательно, но засчитывается"
- [ ] если `has_more_available=false` и весь курс/источники исчерпаны — финальный блок "На сегодня источники исчерпаны"
- [ ] обновить JS-тесты или snapshot шаблона
- [ ] pytest зелёный
